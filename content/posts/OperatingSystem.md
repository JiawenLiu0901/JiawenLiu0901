...
线程： 由于线程共享相同的地址空间，线程的内存消耗相对较小。
	...
	- 线程的栈和堆有什么区别？
		...
		- 线程的堆和栈用于储存程序运行时的数据和变量。
			...
			- 分配方式：
栈： 栈是由系统自动分配和释放的，是一种局部变量存储的地方。每个线程都有自己的栈，用于存储函数调用时的局部变量、函数参数、返回地址等信息。栈的分配和释放是由程序运行时系统自动管理的。
堆： 堆是由程序员手动分配和释放的内存区域。在堆上分配的内存需要程序员负责管理，手动释放以防止内存泄漏。
空间大小：
栈： 栈的空间相对较小，通常由系统预分配，并在运行时动态调整。因为栈的大小是有限的，所以过度使用栈可能导致栈溢出。
堆： 堆的空间相对较大，通常受限于系统的虚拟内存大小。在堆上可以动态分配较大的内存块。
生命周期：
栈： 栈上的数据的生命周期与其所在函数的执行周期相同。当函数执行完毕时，栈上的局部变量会被自动释放。
堆： 堆上的数据的生命周期可以由程序员控制，需要手动分配和释放。
访问速度：
栈： 栈上的数据的访问速度相对较快，因为栈是一种有序的数据结构，数据的存取是直接的。
堆： 堆上的数据的访问速度相对较慢，因为它是一种动态分配的、无序的内存区域。
碎片问题：
栈： 由于栈的分配和释放是自动进行的，不容易产生碎片问题。
堆： 堆上的内存分配和释放可能导致碎片问题，需要额外的管理机制来解决。
...
复杂性： 多线程编程相对于单线程编程更为复杂，需要考虑同步、线程安全、死锁等问题，增加了程序设计和维护的难度
	...
	- 什么是共享内存？如何实现线程间的共享内存？
		...
		- 共享内存时通过内存映射来实现的，进程可以通过共享内存来通信，而共享内存是通过映射相同文件到通信进程的虚拟地址空间实现的。

	- 什么是读写锁？请介绍读写锁的概念、类型、操作以及使用场景。

		- 概念

			- 是一种多线程同步机制，用于控制对共享资源的访问。与普通的互斥锁（Mutex）不同，读写锁允许多个线程同时读取共享资源，但只允许一个线程写入共享资源。

		- 类型

			- 读锁（共享锁）：
多个线程可以同时获取读锁，以读取共享资源。读锁在没有写锁的情况下是共享的，允许多个线程同时持有读锁。

			- 写锁（独占锁）：
写锁是独占的，一次只允许一个线程持有写锁，用于修改共享资源。当一个线程持有写锁时，其他线程无法获取读锁或写锁，直到写锁被释放。
	...
	- 什么是同步和异步？请分别介绍同步和异步的概念、优缺点以及使用场景。
		...
		- 同步
			...
			- 概念
				...
				- 直接制约关系，是指为了完成某种任务而建立的两个或者多个进程，这些进程因为需要在某些位置上协调他们的工作次序而等待、传递信息所产生的制约关系。
		...
		- 异步
			...
			- 概念
				...
				- 异步是指任务不按照顺序执行，一个任务的开始不必等待上一个任务的结束。在异步操作中，任务的执行是非阻塞的，可以在后台执行。
			...
			- 使用场景
				...
				- 不涉及共享资源，没有时序上的严格关系，常用于IO操作等耗时操作
